            ...
            <Container className="relative z-10">
                ...
                <div className="space-y-3">
                    {steps.map((step, index) => (
                        <div
                            key={index}
                            className="group flex items-center gap-5 md:gap-8 bg-white border-2 border-brand-blue/12 rounded-tl-xl rounded-br-xl px-6 py-5 transition-all duration-200"
                        >
                            {/* Large decorative number */}
                            <span className="text-4xl md:text-5xl font-display font-extrabold text-brand-blue/8 tabular-nums flex-shrink-0 transition-colors duration-200 w-14 text-center leading-none select-none">
                                {step.number}
                            </span>

                            {/* Icon badge */}
                            <div className="w-10 h-10 rounded-tl-lg rounded-br-lg bg-brand-blue flex items-center justify-center flex-shrink-0 text-white shadow-[2px_2px_0px_0px_rgba(10,22,48,0.28)] transition-colors duration-200">
                                <step.icon size={18} />
                            </div>
