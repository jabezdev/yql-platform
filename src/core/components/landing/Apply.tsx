                    ...
                    {steps.map((step, index) => {
                        ...
                        return (
                            <div key={index} className="relative flex flex-col items-center text-center">
                                {/* Connector line */}
                                {!isLast && (
                                    <div className="hidden md:block absolute top-6 left-[calc(50%+24px)] right-0 h-px bg-white/15 z-0" />
                                )}

                                <div
                                    className={`w-12 h-12 rounded-tl-xl rounded-br-xl flex items-center justify-center font-display font-extrabold text-sm mb-3 flex-shrink-0 relative z-10 shadow-[3px_3px_0px_0px_rgba(0,0,0,0.25)] ${
                                        isLast
                                ...
                                <p
                                    className={`text-[11px] font-display font-bold leading-snug ${
                                        isLast ? 'text-brand-yellow' : 'text-white/60'
                                    }`}
                                >
                                    {step.title}
                                </p>
