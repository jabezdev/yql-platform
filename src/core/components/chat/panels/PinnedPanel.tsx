...
export function PinnedPanel({ channelId }: PinnedPanelProps) {
    ...
    if (messages.length === 0) {
        return (
            <div className="flex flex-col items-center justify-center flex-1 gap-2 text-brand-blueDark/25 px-4">
    ...
    return (
        <div className="flex flex-col gap-0 overflow-y-auto custom-scrollbar flex-1">
            {messages.map((msg) => (
                <div
                    key={msg._id}
                    className="group px-4 py-3 border-b border-brand-blueDark/6 hover:bg-brand-bgLight/60 transition-colors"
                >
                    <div className="flex items-start gap-2">
                        ...
                        <button
                            ...
                            title="Unpin message"
                            className="opacity-0 group-hover:opacity-100 p-1.5 rounded-lg text-brand-blueDark/30 hover:text-red-500 hover:bg-red-50 transition-all flex-shrink-0"
                        >
                            <Pin size={13} />
                        </button>
                    </div>
