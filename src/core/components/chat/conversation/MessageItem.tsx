...
export function MessageItem({ message, isGrouped = false }: MessageItemProps) {
    ...
    return (
        <div
            className={`group relative flex gap-3 px-4 hover:bg-brand-blueDark/[0.02] transition-colors ${isGrouped ? "pt-0.5" : "pt-3"}`}
        ...
        >
            ...
            {!message.isDeleted && !isEditing && (
                <button
                    className="flex lg:hidden absolute right-3 top-1.5 p-1.5 rounded-lg text-brand-blueDark/20 active:text-brand-blueDark active:bg-brand-bgLight transition-colors z-10"
            ...
            <div className="w-9 flex-shrink-0 pt-0.5">
                {!isGrouped ? (
                    ...
                    <span className="block w-full text-center text-[10px] text-brand-blueDark/20 opacity-0 group-hover:opacity-100 transition-opacity leading-9">
            ...
            <div className="flex-1 min-w-0">
                {!isGrouped && (
                    <div className="flex items-baseline gap-2 mb-0.5">
                        <span className="text-sm font-bold text-brand-blueDark">{authorName}</span>
                        <span className="text-[10px] text-brand-blueDark/35 font-medium">
                            {format(new Date(message._creationTime), "h:mm a")}
                        </span>
                        {message.isPinned && (
                            <span className="text-[10px] text-brand-blue/60 flex items-center gap-0.5">
                                <Pin size={10} /> pinned
                            </span>
                        )}
                        {message.isEdited && (
                            <span className="text-[10px] text-brand-blueDark/30 italic">(edited)</span>
                        )}
                    </div>
                )}
                        ...
                        {message.attachments.map((att) => (
                            att.mimeType.startsWith("image/") ? (
                                <img
                                    ...
                                    src={att.url ?? ""}
                                    alt={att.filename}
                                    className="max-h-64 w-full sm:max-w-xs rounded-tl-lg rounded-br-lg border-2 border-brand-blueDark/10 object-cover"
                                />
                            ) : (
                                <a
                                    key={att.storageId}
                                    ...
                                    className="flex items-center gap-2 px-3 py-2 bg-brand-bgLight border-2 border-brand-blueDark/10 rounded-tl-lg rounded-br-lg text-xs font-medium text-brand-blue hover:border-brand-blue/40 transition-colors"
                                >
                                    {att.filename}
                                </a>
                            )
                            ...
                            return (
                                <button
                                    ...
                                    className={`flex items-center gap-1 px-2 py-0.5 rounded-full border text-xs font-medium transition-all ${
                                        hasReacted
                                            ? "bg-brand-blue/10 border-brand-blue/40 text-brand-blueDark"
                                            : "bg-white border-brand-blueDark/15 text-brand-blueDark/60 hover:border-brand-blue/30"
                                    }`}
                                >
                                    <span>{r.emoji}</span>
                                    <span>{r.count}</span>
                ...
                {!message.threadRootMessageId && (message.threadReplyCount ?? 0) > 0 && (
                    <button
                        ...
                        className="flex items-center gap-2 mt-1.5 text-xs text-brand-blue font-semibold hover:underline"
                    >
            ...
            {showActions && !message.isDeleted && (
                <div className="absolute right-4 -top-3 flex items-center gap-0.5 bg-white border-2 border-brand-blueDark/10 rounded-tl-xl rounded-br-xl shadow-[2px_2px_0px_0px_rgba(10,22,48,0.08)] px-1 py-0.5 z-10">
                    ...
                    {(user?.role === "T3" || user?.role === "T2" || user?.role === "T1" || user?.role === "Super Admin") && (
                        <ActionBtn title="Move to channel" onClick={() => { setShowMoveModal(true); setShowActions(false); }}>
                            <MoveRight size={14} />
                        </ActionBtn>
                    )}
                    {(isOwn || user?.role === "T3" || user?.role === "T2" || user?.role === "T1" || user?.role === "Super Admin") && (
