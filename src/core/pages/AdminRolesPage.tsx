...
export default function AdminRolesPage() {
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="bg-white border-2 border-brand-blue shadow-[4px_4px_0px_0px_rgba(57,103,153,0.15)] rounded-tl-2xl rounded-br-2xl overflow-hidden">
                <div className="grid gap-0 divide-y-2 divide-brand-blue">
                    {users.map((user: any) => (
                        <div key={user._id} className="p-5 md:p-6 flex flex-col md:flex-row md:items-center justify-between hover:bg-brand-bgLight transition-colors">
                            <div className="mb-4 md:mb-0">
                                <p className="font-extrabold font-display text-brand-blue text-lg">{user.name}</p>
                                <p className="text-sm text-brand-darkBlue/70 font-medium">{user.email}</p>
                            </div>
                            <div className="flex flex-col sm:flex-row items-center gap-4">
                                <div className="flex flex-col w-full sm:w-auto">
                                    <span className="text-[10px] font-extrabold text-brand-blue/50 uppercase tracking-widest mb-1.5 ml-1">Primary Role</span>
                                    <select
                                        className="p-2 border-2 border-brand-blue/20 focus:border-brand-blue focus:shadow-[2px_2px_0px_0px_rgba(57,103,153,0.3)] transition-all rounded-lg text-sm bg-brand-bgLight/50 outline-none focus:ring-0 font-bold text-brand-blue cursor-pointer appearance-none min-w-[140px]"
                                        value={user.role}
                                ...
                                {checkStaff(user.role as Role) ? (
                                    <div className="flex flex-col w-full sm:w-auto">
                                        <span className="text-[10px] font-extrabold text-brand-lightBlue/50 uppercase tracking-widest mb-1.5 ml-1">Special Designations</span>
                                        <select
                                            className="p-2 border-2 border-brand-lightBlue/20 focus:border-brand-lightBlue focus:shadow-[2px_2px_0px_0px_rgba(37,99,235,0.3)] transition-all rounded-lg text-sm bg-brand-lightBlue/5 outline-none focus:ring-0 font-bold text-brand-lightBlue cursor-pointer appearance-none min-w-[140px]"
                    ...
                    {users.length === 0 && <p className="text-center text-brand-blue/50 py-12 font-bold bg-brand-bgLight/30">No users found in the system yet.</p>}
                </div>
            </div>
        </div>
    );
