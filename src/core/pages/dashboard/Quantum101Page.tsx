...
When we measure a qubit, its superposition **collapses** to either 0 or 1 with probabilities |α|² and |β|² respectively. This ability to be in multiple states simultaneously is what gives quantum computers their potential for parallelism.`,
    ...
    },
    {
        id: "entanglement",
        title: "Entanglement",
        icon: <GitBranch size={20} className="text-brand-lightBlue" />,
...
Quantum circuits are sequences of gates applied to qubits, ending in a measurement. Unlike classical circuits, all gates are reversible except measurement.`,
    ...
    },
    {
        ...
        icon: <FlaskConical size={20} className="text-brand-lightBlue" />,
...
function ModuleCard({ mod }: { mod: Module }) {
    ...
    return (
        <div className={`bg-white border-2 border-brand-blue rounded-tl-2xl rounded-br-2xl shadow-[4px_4px_0px_0px_rgba(57,103,153,0.15)] transition-all ${open ? "shadow-[4px_4px_0px_0px_rgba(57,103,153,0.6)]" : "hover:shadow-[4px_4px_0px_0px_rgba(57,103,153,0.5)] hover:-translate-y-0.5"}`}>
            <button
                onClick={() => setOpen(!open)}
                className="w-full text-left p-5 flex items-start gap-4"
            >
                <div className="w-10 h-10 bg-brand-bgLight border-2 border-brand-blue/20 rounded-tl-lg rounded-br-lg flex items-center justify-center shrink-0 mt-0.5">
                    {mod.icon}
                </div>
                <div className="flex-1 min-w-0">
                    <h3 className="font-display font-extrabold text-brand-blue text-lg">{mod.title}</h3>
                    <p className="text-sm text-brand-blue/60 font-medium mt-0.5 leading-relaxed">{mod.summary}</p>
                </div>
                <div className="shrink-0 mt-1">
                    {open ? <ChevronDown size={20} className="text-brand-lightBlue" /> : <ChevronRight size={20} className="text-brand-blue/40" />}
                </div>
            </button>

            {open && (
                <div className="border-t-2 border-brand-blue/10 p-5 space-y-6 animate-in fade-in duration-150">
                    <div className="prose-custom text-sm text-brand-blue/80 leading-relaxed whitespace-pre-line">
                        {mod.content.split("\n").map((line, i) => {
                    ...
                    <div className="bg-brand-bgLight border-2 border-brand-blue/10 rounded-xl p-4">
                        <p className="text-[10px] font-extrabold uppercase tracking-widest text-brand-blue/40 mb-3">Key Terms</p>
                        <div className="space-y-2">
                            {mod.keyTerms.map(kt => (
                                <div key={kt.term} className="flex gap-3">
                                    <span className="font-bold text-brand-blue text-sm shrink-0 w-36">{kt.term}</span>
                                    <span className="text-sm text-brand-blue/60 font-medium leading-relaxed">{kt.definition}</span>
                                </div>
                            ))}
                        </div>
                    </div>
                    ...
                    {mod.readMore && (
                        <a
                            ...
                            target="_blank"
                            rel="noreferrer"
                            className="inline-flex items-center gap-2 text-sm font-bold text-brand-lightBlue hover:underline"
                        >
    ...
    return (
        <div className="w-full max-w-3xl mx-auto space-y-6 pb-8">
            <div className="flex items-start gap-4">
                <div className="w-14 h-14 bg-brand-yellow/20 border-2 border-brand-blue rounded-tl-xl rounded-br-xl flex items-center justify-center shadow-[3px_3px_0px_0px_rgba(57,103,153,0.3)] shrink-0 mt-1">
                    <Atom size={28} className="text-brand-blue" strokeWidth={2} />
                </div>
                <PageHeader
                    title="Quantum 101"
            ...
            <div className="bg-brand-lightBlue/5 border-2 border-brand-lightBlue/20 rounded-xl p-4 text-sm font-medium text-brand-blue/70 leading-relaxed">
