        ...
        <DashboardPage className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <div className="space-y-6">
                    <DashboardSectionTitle 
                        action={<Button variant="ghost" size="sm" className="text-brand-blue/40">View All</Button>}
                    >
                        Core Modules
                    </DashboardSectionTitle>
                    <div className="grid grid-cols-1 gap-4">
                        {[
                            ...
                            <DashboardCard key={i} variant="glass" className="group cursor-pointer">
                                <div className="flex items-center gap-4">
                                    <div className={`w-11 h-11 rounded-tl-lg rounded-br-lg border-2 flex items-center justify-center transition-all duration-300 ${
                                        module.complete 
                                            ...
                                            : 'bg-white dark:bg-white/5 border-brand-blue/20 dark:border-white/20 text-brand-blue dark:text-white/40 group-hover:bg-brand-blue dark:group-hover:bg-brand-yellow group-hover:border-brand-blue dark:group-hover:border-brand-yellow group-hover:text-white dark:group-hover:text-brand-blue'
                                    }`}>
                                        <module.icon size={20} />
                                    </div>
                                    <div className="flex-1 min-w-0">
                                        <h4 className="font-bold text-brand-blue dark:text-white group-hover:text-brand-lightBlue dark:group-hover:text-brand-yellow transition-colors truncate">{module.title}</h4>
                                        <div className="flex items-center gap-2 mt-0.5">
                                            <p className="text-[10px] font-black uppercase tracking-widest opacity-40 dark:opacity-30">{module.duration}</p>
                ...
                <div className="space-y-6">
                    ...
                    <DashboardCard variant="static" className="h-fit">
                            ...
                            {[
                                ...
                                <div key={i} className="flex items-center justify-between group cursor-pointer">
                                    <div className="flex items-center gap-3">
                                        <div className="p-3 bg-brand-blue/5 dark:bg-white/5 rounded-lg border border-brand-blue/10 dark:border-white/10 text-brand-blue/40 dark:text-white/40 group-hover:bg-brand-blue/10 dark:group-hover:bg-brand-yellow/20 group-hover:text-brand-blue dark:group-hover:text-brand-yellow transition-colors">
                                            <res.icon size={14} />
                                        </div>
                                        <span className="text-sm font-bold text-brand-blue dark:text-white/90 underline decoration-brand-blue/10 dark:decoration-white/10 underline-offset-4 group-hover:decoration-brand-blue dark:group-hover:decoration-brand-yellow transition-all">
                                            {res.title}
                                        </span>
                                    </div>
                                    <span className="text-[10px] font-mono font-bold opacity-30 dark:opacity-20 group-hover:opacity-60 transition-opacity">{res.size}</span>
                                </div>
                            ))}
                        </div>
                        <Button variant="ghost" fullWidth className="mt-8 border-2 border-brand-blue/5 dark:border-white/10 text-brand-blue/50 dark:text-white/40 hover:border-brand-blue/30 dark:hover:border-white/30 hover:bg-white dark:hover:bg-white/5">
                            Open Full Vault
                        </Button>
                    </DashboardCard>
                </div>
