...
import { assertCanReadChannel, assertCanReadMessage } from "./lib/access";
import { checkPostRateLimit, logRateLimitEvent, shouldBypassRateLimit } from "./lib/rateLimit";
...
export const sendMessage = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        // Rate limiting
        if (!shouldBypassRateLimit(user.role as string)) {
            const hasQuota = await checkPostRateLimit(ctx, user._id, args.channelId);
            if (!hasQuota) {
                await logRateLimitEvent(ctx, user._id, args.channelId, "message_limit");
                throw new ConvexError({ code: "TOO_MANY_REQUESTS", message: "You are sending messages too fast" });
            }
        }
...
export const createMediaItems = mutation({
    ...
    handler: async (ctx, { messageId, channelId, attachments, urlPreviews, uploadedBy: _uploadedBy }) => {
        void _uploadedBy; // Silence unused warning
        const user = await requireUser(ctx);
        const message = await ctx.db.get(messageId);
        if (!message) {
